...
Adds the ARIMA model used for seasonal adjustment directly onto a ggplot.
...
library(ggdemetra)
library(forecast)
arimadata <- worldbankdata |>
  filter(Country == "Sri Lanka",
         Year >= 2013, Year <= 2021) |>
  select(Country, Year, Electricity)

ggplot(arimadata, aes(x = Year, y = Electricity, color = Country)) +
  geom_line() + # Plots the raw data 
 geom_arima(data = arimadata, method = "X13", geom = "label",
               x_arima = - Inf, y_arima = -Inf, 
               vjust = -1, hjust = -0.1,
               message = FALSE)   # ARIMA specifications 

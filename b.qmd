# geom_b {#sec-b}

```{r, message=FALSE, warning=FALSE,echo=FALSE}
library(tidyverse)
library(drone)
library(patchwork)
data("worldbankdata")
library(viridis)
```

## geom_bar{#bar}

### Package

ggplot2 [@ggplot2]

### Description

Draw a bar proportional to the specified number. For example, number of cases or user defined number.

### The statistical transformation to use on the data for this layer 

  `stat_count` - This the default statistics layer. It counts number of cases in each group. 
  
  `stat_identify` - It plots the data as it is.

### See also

[geom_col](#col)

#### With count 

```{r, fig.cap="Illustration of geom_bar to create a bar chart", message=FALSE, warning=FALSE}
worldbankdata |>
  dplyr::select(c("Year", "Income")) |>
  dplyr::filter(Year==2021) |>
  ggplot(aes(x = Income)) + 
  geom_bar()

```

```{r, fig.cap="Illustration of geom_bar to create a stacked bar chart", message=FALSE, warning=FALSE,}
worldbankdata |>
  dplyr::select(c("Year", "Income", "Region")) |>
  dplyr::filter(Year==2021) |>
  ggplot(aes(x = Income, fill=Region)) + 
  geom_bar()
```

```{r, fig.cap="Illustration of geom_bar to create a cluster bar chart", message=FALSE, warning=FALSE}
worldbankdata |>
  dplyr::select(c("Year", "Income", "Region")) |>
  dplyr::filter(Year==2021) |>
  ggplot(aes(x = Income, fill=Region)) + 
  geom_bar(position = "dodge")
```



#### With identity

```{r, message=FALSE, warning=FALSE}
dfbar <- data.frame(class=c("A", "B"),  income = c(100, 200))
ggplot(dfbar, aes(class, income)) +
  geom_bar(stat="identity")
```

## geom_bin_2d{#bin2d}

### Package

ggplot2 [@ggplot2]

### Description

Divides the Cartesian plane created by x-variable and y-variable into rectangles (2D histogram), counts the number of observations in each rectangle. Only the observations with rectangles are filled according to the number of observations.

### Understandable aesthetics

**required aesthetics**

`x`, `y`, 

**optional aesthetics**

`fill`, `group`

### The statistical transformation to use on the data for this layer 

`stat_bin2d`

### See also 

[geom_bin2d](#bin2d), [geom_point](#point)

### Example

```{r, fig.cap="Illustration of using geom_bin_2d", message=FALSE, warning=FALSE}
ggplot(worldbankdata, aes(y = Cooking, x=Electricity)) +
  geom_bin_2d() + 
  scale_fill_viridis() +
  theme(aspect.ratio = 1)
```

```{r, fig.cap="Illustration of changing bins in geom_bin_2d", message=FALSE, warning=FALSE}
ggplot(worldbankdata, aes(y = Cooking, x=Electricity)) +
  geom_bin_2d(bins=20) + 
  scale_fill_viridis() +
  theme(aspect.ratio = 1)
```

## geom_bin2d_pattern{#bin2dpattern}

### Package

ggpattern[@ggpattern]

### Description

Divides the Cartesian plane created by x-variable and y-variable into rectangles (2D-Histogram), counts the number of observations in each rectangle. Only the observations with rectangles are filled with a pattern.

### Understandable aesthetics

`pattern_fill` (pattern_* - for mapping variables under aesthetics), `pattern` (to set a patten, for example pattern='stripe'), `fill`, `colour`

### The statistical transformation to use on the data for this layer 


### See also

[geom_bin2d](#bin2d), [geom_point](#point)

```{r, fig.cap="Illustration of using geom_bin_2d", message=FALSE, warning=FALSE}
ggplot(worldbankdata, aes(y = Cooking, x=Electricity)) +
  ggpattern::geom_bin2d_pattern(bins=20) + 
  ggpattern::scale_pattern_fill_viridis_c()
  theme(aspect.ratio = 1)
```

## geom_bin2d{#bin2d}


### Package

ggplot2 [@R-ggplot2]

### Description 

Divides the Cartesian plane created by x-variable and y-variable into rectangles, counts the number of observations in each rectangle. Only the observations with rectangles are filled according to the number of observations.

### Understandable aesthetics

`x`, `y`, `fill`, `group`

### The statistical transformation to use on the data for this layer 

`stat_bin_2d`

**See also: ** [geom_bin_2d](#bin_2d), [geom_point](#point)

**Example:**

```{r, fig.cap="Illustration of using geom_bin_2d", message=FALSE, warning=FALSE}
ggplot(worldbankdata, aes(y = Cooking, x=Electricity)) +
  geom_bin2d() + 
  theme(aspect.ratio = 1) +
  scale_fill_viridis()
```


## geom_blank{#blank}

**Package: ** ggplot2 [@R-ggplot2]

**Description: ** Draws nothing.

## geom_boxplot{#boxplot}


**Package: ** ggplot2 [@R-ggplot2]

**Description: ** Draw a bar proportional to the specified number. For example, number of cases or user defined number.

### The statistical transformation to use on the data for this layer 

  `stat_boxplot` - This the default statistics layer. This computes minimum, maximum, median, first quartile ($Q1$), third quartile ($Q3$), upper whisker extends up to $Q3 + 1.5 \times IQR$ and lower whisker extends up to $Q1 - 1.5 \times IQR$, where $IQR = Q3 - Q1$. In a notched box plot, it creates 95\% confidence interval for mean.
  

**See also: ** [geom_col](#col)

```{r, fig.cap="Illustration of using geom_boxplot", message=FALSE, warning=FALSE}
worldbankdata |>
  dplyr::filter(Year == 2021) |>
  dplyr::select(Cooking) |>
ggplot(aes(y = Cooking, x=factor(0))) + 
  geom_boxplot() + 
  theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank())
```

```{r geomboxparam, fig.cap="Illustration of using geom_boxplot with changing outliers and adding a notch to create notched box plot.", message=FALSE, warning=FALSE}
worldbankdata |>
  dplyr::filter(Year == 2021) |>
  dplyr::select(Cooking) |>
ggplot(aes(y = Cooking, x=factor(0))) +  
geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=TRUE) + 
  theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank())

```




...
\title{Data Visualisation \textbf{Geom}etrie\emph{s} Encyclopedia}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{geoms in the grammar of graphics (ggplot)}
...
available at: https://github.com/thiyangt/drone
...
percentage of people access to electricity in lower incom (L), lower
...
that we use and statistics we visualise the plots. For example, in chart
A: all values are represented using individual points. In chart H: The
range is denoted using a blue line and mean is represented using a red
dot.
